...
- 需授予定位权限，并配置**高德 Web 服务 Key** 以搜索地点
- 可添加多个避让点；**触发半径**在设置页「管理避让地点」中用滑条调节（**5～500 米**，默认 10 米）
- 页面展示**当前位置**（经纬度、精度）以及距最近避让点的距离，便于确认是否已进入范围
- **绿色模式**（可选）：进入半径内后，纯度在 **R13** 与 **仅 Sketchy** 间随机，不再固定锁 R13
