...
object ProcessBridgePrefs {
    ...
    fun sync(context: Context, s: AppSettings, writeEnabled: Boolean = true) {
        val ed = context.applicationContext
    ...
    private fun enabledFile(context: Context): java.io.File =
        java.io.File(context.applicationContext.filesDir, "jhsy_enabled.txt")
    ...
    fun enabled(context: Context): Boolean {
        val fromFile = runCatching {
            enabledFile(context).takeIf { it.exists() }?.readText()?.trim()
        }.getOrNull()
        return when (fromFile) {
            "1" -> true
            "0" -> false
            else -> sp(context).getBoolean("enabled", false)
        }
    }
