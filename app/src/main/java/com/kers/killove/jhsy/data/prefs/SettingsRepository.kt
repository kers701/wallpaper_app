...
class SettingsRepository(private val context: Context) {
    ...
    val settingsFlow: Flow<AppSettings> = context.dataStore.data.map { p ->
        ...
        AppSettings(
            enabled = runCatching { ProcessBridgePrefs.enabled(context) }.getOrElse { p[Keys.ENABLED] ?: false },
            ...
            target = runCatching { WallpaperTarget.valueOf(p[Keys.TARGET] ?: "Both") }.getOrDefault(WallpaperTarget.Both),
            resolutionMode = ResolutionMode.entries.find { it.code == (p[Keys.RES_MODE] ?: "zsy") } ?: ResolutionMode.Device,
            ...
            blacklistPackages = if (ProcessBridgePrefs.blacklistFileExists(context)) ProcessBridgePrefs.readBlacklist(context) else splitLines(p[Keys.BLACKLIST] ?: ""),
            ...
            avoidanceLocationsJson = ProcessBridgePrefs.effectiveAvoidLocationsJson(context, p[Keys.AVOID_LOCS] ?: "[]"),
