...
2. 设置关键词或开启跃迁模式；纯度筛选（可选）、类别、分辨率按喜好选择。
...
• 纯度筛选（可关）：关闭时每次更换在全部级别中完全随机。
  开启后用三个按钮多选组合：
  - 保守级（SFW）：适合日常、公共场合、未成年相关场景需自行合规判断
  - 模糊级（Sketchy）：轻度暧昧/暴露边缘，不适合严格公开场合
  - 限制级（NSFW）：成人向内容，仅限私密环境与合法成年用户
  组合示例：仅模糊级=010；仅限制级=001；模糊+限制=011；保守+限制=101；保守+模糊=110。
• 类别 / 分辨率：过滤结果；设备自适应会按真实分辨率请求。
• 横竖屏过滤：无过滤 / 仅竖屏 / 仅横屏。
...
""".trimIndent())
        ...
        GuideSection("命运先机（4.0）", """
• 总开关在配置页「命运先机」。
• 「命运先机配置」可添加多条规则：开始/结束时间（24 小时）、生效星期、先机模式。
• 模式：健康（模糊级）/ 心跳（模糊+限制）/ 用户配置 / 自定义纯度。
• 每条可勾选是否生效，并设优先级 0～999；带置信度。
• 冲突判定：先优先级（越小越优先）→ 再置信度（越大越优先）→ 再修改/创建时间。
• 命中时纯度按规则劫持；配置页显示「纯度选择已被命运劫持-配置名-（纯度）」。
• 通知栏切换模式：若已劫持，询问是否强制恢复。「是」本时段跳过且置信度−1 并恢复原模式；「否」继续劫持且置信度+1。下一段生效时间再正常命中。
• 页面底部「保存全部命运先机修改」写入本地。
""".trimIndent())

        GuideSection("虚妄模式", """
• 需先开启跃迁与湮灭后才可显示并开启。
• 过滤顺序：跃迁清洗本次用词 → 虚妄（含本次用词的相关标签）→ 湮灭（本纪元用过的词，最后执行）。
• 例：用词 girl，则 a girl、girl b 等均入虚妄；其余再交湮灭决定是否进跃迁。
• 「我们从过去走出，应舍弃前尘，从此，身前虚妄，身后亦是虚妄，我们没有过去，也没有未来，但我们却有着千丝万缕的联系」
• 首页/概览显示状态；虚妄关键词卡片默认只显示数量，点击展开详情。
""".trimIndent())
...
【标题】普通 / 健康 / 心跳 三种模式：
• MiroFlweat·普通模式 — 遵循配置：未开纯度筛选则完全随机；开启则用用户勾选的保守/模糊/限制组合
• MiroFlweat·健康模式 — 锁定仅「模糊级」(010)，忽略用户纯度勾选
• MiroFlweat·心跳模式 — 锁定「模糊级+限制级」(011)，忽略用户纯度勾选
• 绿色模式（定位避让区内）— 锁定「保守级+模糊级」(110）
